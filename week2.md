...
ai랑 릴레이 소설 쓰면서 문화생활하기.

**수행 방법**

> 각 수행 방법은 기본적으로 프롬프트를 작성하고 기존에 작성된 소설을 제공한다음 이전 내용을 분석해 소설의 뒷부분을 추가하는 응답을 만들어낸다.
>
> 또한, 릴레이 소설 형식으로 작성한다. 소설은 Day1 부터 진행된 CS지식을 기반으로 소설을 작성한다.

1. AI스스로가 특정 작가의 화풍을 스스로 검색하고 찾아 소설을 작성하도록 만든다
2. 작가의 화풍을 프롬프트에 추가해준다.

**수행 결과**

1. AI 스스로 작가의 화풍을 찾아 분석

```md
미션 2일차, '리눅스'의 날이 밝았다. 하지만 캠프를 감싼 공기는 어제보다 한층 더 무거워졌다. 두 번째 실종자의 자리는 텅 비어 있었고, 그의 모니터에는 검은 터미널 창만이 유령처럼 깜빡이고 있었다. 마지막으로 입력된 명령어는 chmod 777. 모든 파일에 모든 권한을 부여하는, 위험천만한 명령어였다.

그는 호기심이 많았다. 다른 사람의 디렉토리를 엿보는 것이 취미였고, 종종 권한 문제로 다른 이들과 다투곤 했다. 나는 깨달았다. 범인은 그에게 '모든 것을 볼 수 있는 권한'을 미끼로 던진 것이다. 그리고 그 금지된 권한의 끝에서, 그는 자신의 목숨이라는 대가를 치렀다.

마치 시스템의 'root' 권한을 탈취한 해커처럼, 범인은 이 부트캠프라는 폐쇄된 공간을 장악하고 있었다. 우리는 모두 그의 통제하에 놓인 '프로세스'에 불과했다. 언제든 'kill -9' 명령어로 강제 종료될 수 있는. 공포가 뼛속까지 스며들었다. 다음 미션은 무엇일까. 그리고 다음 희생자는.
```

2. 작가의 화풍을 사용자가 제공

```
리눅스 강의가 있던 2일차, 모두가 검은 터미널 창에 낯선 명령어들을 입력하고 있었다. ls, cd, mkdir. 키보드를 두드리는 소리가 강의실을 채웠지만, 내 머릿속은 온통 de-bug.h라는 존재하지 않는 파일명으로 가득 차 있었다. 평범한 오타라고 하기엔 그가 남긴 코드의 모든 것이 너무나도 정교했다.

쉬는 시간을 틈타, 나는 몰래 서버에 접속해 그의 홈 디렉터리로 이동했다. 관리자에게 들키면 안 된다는 생각에 심장이 가볍게 떨려왔다. ls -al 명령어로 숨겨진 파일까지 모두 확인했을 때, 마침내 위화감의 실체를 발견했다. 수많은 파일 속에서 유독 .bash_history 파일의 권한이 r--r----- (440)으로 기묘하게 잠겨 있었다. 자신의 마지막 행적을 기록한 파일을, 그룹 내 다른 누구도 아닌 불특정한 ‘다른 사용자(others)’에게만 읽기 가능하도록 열어둔 것이다.

그는 사라진 게 아니었다. 자신의 흔적을, 마치 밀실에 갇힌 피해자가 남기는 암호처럼 남겨두고 누군가 발견해주길 바랐던 것이다. 이 기묘한 권한 설정은 그가 던지는 두 번째 질문이자, 다음 행선지를 가리키는 이정표였다.
```

**회고**

**작가의 화풍을 AI 스스로 찾아 분석**하는 `1번`의 방법은 하나의 소설 화풍에 국한되지않고 공통적으로 보이는 화풍을 묘사하려고한 점이 확실히 눈에 띄었다. **작가의 화풍을 사용자가 제공**하는 `2번`의 경우는 제공된 컨텍스트 소설 내용 또한 작가의 화풍을 반영함이 보였다. 허나, 작가가 여러 장르를 집필한 경우에 화풍이 조금 더 추상적으로 변하게 되는데 하나의 화풍 컨텍스트만 제공했기 때문에 일반적인 화풍을 묘사해내지는 못하는것 같다.

AI 스스로 찾아보고 분석하도록 프롬프트를 주입시켜 자신이 결과물에 대한 컨텍스트를 만들 수 있도록 이끌어가는것이 중요한것 같다.
...
미션에 집중하다보면 코드에 갇혀 답답하게 느껴질 때가 많았다.<br /> 그럴때 마다 분위기를 환기하고, 재미있는 생각을 할 수 있는 퀘스트인것 같아 선택하게 되었다.
...
- 무릎을 강화해야해서, 다리 스트레칭 4컷 만화를 그려달라고 구체적으로 요청했습니다.
  ...
  > 내가 무릎이 안좋아서 다리 근육을 강화하는 운동이나 스트레칭을 해야 해.<br /> 가볍게 무릎 혹은 다리 스트레칭을 하는 방법을 4컷만화로 하나 그려줘

<img width="50%" alt="image" src="https://github.com/user-attachments/assets/96bb1ce5-4efb-4c25-b728-e6754cf21d5d" />

이전에 재활운동으로 다리 스트레칭을 많이 했었는데, 그림의 동작을 실제로 했었습니다. 뭉친 근육을 풀면서 골반 정렬과 무릎 주변의 근육 밸런스를 개선해줍니다.

특히 오래 앉아있거나 자세가 올바르지 않은 사람들에게 추천하는 스트레칭입니다.

위 그림을 준비하면서, 여러 AI에게 요청했는데 설명이 부족했던 것인지 퀄리티가 너무 떨어져서 직접 그리는게 낫겠다는 생각도 들었습니다. 나의 생각을 잘 표현할 수 있도록 프롬프트를 잘 작성하는게 생각보다 어려워서, 글로 정리하는 연습을 해야 할 것 같습니다.
...
- 릴레이 프로젝트 팀원들의 이름을 주고, 3행시를 지어달라고 부탁했습니다.
  ...
  > <이름 제공> 위 3명의 이름을 가지고 3행시를 지어야 해부정적인 내용은 안되고 최대한 긍정적인 내용으로 부탁해또한, 진지한 것보다는 재미있는 요소가 들어가게 3행시를 지어 줘
